...
def get_dashboard_stats():
    try:
        ...
        # 初始化平台统计数据
        platform_stats = {"kuaishou": 0, "douyin": 0, "channels": 0, "xiaohongshu": 0}
        ...
        task_stats = {"total": 0, "completed": 0, "inProgress": 0, "failed": 0, "waiting": 0}
        ...
        task_trend = {
            "xAxis": [],
            "series": [{"name": "完成任务", "data": []}, {"name": "失败任务", "data": []}],
        }
        ...
        for row in recent_tasks_rows:
            ...
            try:
                task_dict["account_list"] = (
                    json.loads(row["account_list"]) if row["account_list"] else []
                )
            except (json.JSONDecodeError, TypeError):  # Handle invalid JSON
                task_dict["account_list"] = []
        ...
        print(f"获取仪表盘数据失败: {e}")
        return jsonify({"code": 500, "msg": f"获取数据失败: {str(e)}", "data": None}), 500
    finally:
        if "conn" in locals():
            conn.close()
