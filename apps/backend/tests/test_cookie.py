...
class TestCookie:
    ...
    def test_upload_cookie_success(self, mock_path_class, mock_sqlite3, mock_db_manager):
        ...
        with self.app.test_client() as client:
            ...
            response = client.post(
                "/uploadCookie",
                data={"id": "1", "platform": "xhs", "file": (BytesIO(b'{"test": "cookie"'))},
            )
    ...
    def test_upload_cookie_missing_file(self):
        ...
        with self.app.test_client() as client:
            response = client.post("/uploadCookie", data={"id": "1", "platform": "xhs"})
    ...
    def test_upload_cookie_missing_params(self):
        ...
        with self.app.test_client() as client:
            response = client.post("/uploadCookie", data={"id": "1"})
        ...
        with self.app.test_client() as client:
            response = client.post("/uploadCookie", data={"platform": "xhs"})
    ...
    def test_upload_cookie_account_not_found(self, mock_sqlite3, mock_db_manager):
        ...
        with self.app.test_client() as client:
            response = client.post(
                "/uploadCookie", data={"id": "999", "platform": "xhs"}  # 不存在的账号ID
            )
    ...
    def test_upload_cookie_empty_filename(self):
        ...
        with self.app.test_client() as client:
            response = client.post(
                "/uploadCookie", data={"id": "1", "platform": "xhs", "file": (BytesIO(b"{}"), "")}
            )
            assert response.status_code in [400, 500]
    ...
    def test_upload_cookie_non_json(self):
        ...
        with self.app.test_client() as client:
            response = client.post(
                "/uploadCookie",
                data={"id": "1", "platform": "xhs", "file": (BytesIO(b"test"), "test.txt")},
            )
            assert response.status_code in [400, 500]
    ...
    def test_upload_cookie_with_path_creation(
        self, mock_file_save, mock_db_manager, mock_sqlite, mock_path_class
    ):
        """测试上传Cookie时路径创建"""
    ...
    @patch("src.routes.cookie.COOKIES_DIR")
    @patch("src.routes.cookie.send_from_directory")
    @patch("src.routes.cookie.Path")
    def test_download_cookie_success(self, mock_path_class, mock_send, mock_cookies_dir):
        # Mock COOKIES_DIR behavior
        mock_file_path = MagicMock()
        ...
        # Configure COOKIES_DIR / file_path -> mock_file_path
        mock_cookies_dir.__truediv__.return_value = mock_file_path
        # Configure resolve
        mock_cookies_dir.resolve.return_value = mock_cookies_dir
        mock_file_path.resolve.return_value = mock_file_path

        mock_path_class.return_value = mock_file_path  # fallback if Path used
    ...
    def test_download_cookie_illegal_path(self, mock_path_class):
        ...
        with self.app.test_client() as client:
            ...
            assert data["code"] == 500 and (
                "非法" in data["msg"] or "failed" in data["msg"].lower()
            )
