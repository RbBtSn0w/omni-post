...
class TestGroupRoutes:
    def test_create_group(self, client, mock_db_manager):
        ...
        response = client.post(
            "/createGroup", json={"name": "Test Group", "description": "A test group"}
        )
    ...
    def test_update_group(self, client, mock_db_manager):
        ...
        # Update
        response = client.put(
            f"/updateGroup/{group_id}", json={"name": "New Name", "description": "Updated"}
        )
        assert response.status_code == 200
    ...
    def test_delete_group_with_accounts(self, client, mock_db_manager):
        ...
        # Create group
        client.post("/createGroup", json={"name": "Busy Group"})
    ...
    def test_delete_group_with_accounts_integrated(self, client, mock_db_manager, temp_db_path):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO user_info (type, filePath, userName, group_id) VALUES (1, 'path', 'User1', ?)",
            (group_id,),
        )
        conn.commit()
        conn.close()
    ...
    def test_update_nonexistent_group(self, client, mock_db_manager):
        ...
        response = client.put(
            "/updateGroup/99999", json={"name": "New Name", "description": "test"}
        )
        assert response.status_code == 200
        data = response.get_json()
    ...
    def test_update_group_duplicate_name(self, client, mock_db_manager):
        ...
        response = client.put(
            f"/updateGroup/{group_id}", json={"name": "Group A", "description": "test"}
        )
        assert response.status_code == 200
        data = response.get_json()
        assert data["code"] == 400
    ...
    def test_get_group_accounts(self, client, mock_db_manager, temp_db_path):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO user_info (type, filePath, userName, group_id) VALUES (1, 'path1', 'User1', ?)",
            (group_id,),
        )
        cursor.execute(
            "INSERT INTO user_info (type, filePath, userName, group_id) VALUES (2, 'path2', 'User2', ?)",
            (group_id,),
        )
        conn.commit()
        conn.close()
