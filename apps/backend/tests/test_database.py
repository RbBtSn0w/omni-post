...
class TestDatabase:
    ...
    def test_database_path_config(self):
        ...
        assert (
            create_table_db_file == expected_path
        ), f"数据库路径配置错误，预期: {expected_path}，实际: {create_table_db_file}"
    ...
    def test_database_write_read(self):
        ...
        cursor.execute(
            "INSERT INTO user_info (userName, type, filePath, status) VALUES (?, ?, ?, ?)",
    ...
    def test_db_manager_methods(self):
        """测试 DatabaseManager 类的方法"""
        from src.db.db_manager import db_manager

        # 测试 get_data_dir
        data_dir = db_manager.get_data_dir()
        assert isinstance(data_dir, Path), "get_data_dir 应该返回 Path 对象"
        assert data_dir.exists(), f"数据目录不存在: {data_dir}"

        # 测试 get_db_path
        db_path = db_manager.get_db_path()
        assert isinstance(db_path, Path), "get_db_path 应该返回 Path 对象"
        assert db_path.name == "database.db", "数据库文件名应该为 database.db"
