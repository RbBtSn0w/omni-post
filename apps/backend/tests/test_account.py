...
class TestAccount:
    ...
    @patch("src.db.db_manager")
    @patch("sqlite3.connect")
    def test_getAccountStatus_success(
        self, mock_sqlite_connect, mock_db_manager, mock_check_cookie
    ):
        """测试成功获取单个账号状态"""
        # 配置模拟返回值
        mock_db_manager.get_db_path.return_value = "mock_db_path"
    ...
    @patch("src.db.db_manager")
    @patch("sqlite3.connect")
    def test_getAccountStatus_invalid(
        self, mock_sqlite_connect, mock_db_manager, mock_check_cookie
    ):
        """测试获取无效账号状态"""
        # 配置模拟返回值
    ...
    def test_updateUserinfo_success(self, mock_sqlite_connect, mock_db_manager):
        ...
        with self.app.test_client() as client:
            response = client.post(
                "/updateUserinfo", json={"id": 1, "type": "xhs", "userName": "更新后的小红书账号"}
            )
