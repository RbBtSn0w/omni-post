...
class TestDashboard:
    ...
    def test_get_dashboard_stats_normal(self, mock_db_manager, mock_sqlite_connect):
        ...
        def create_mock_row(data, keys):
            ...
            row.__getitem__ = lambda self, key: (
                dict(zip(keys, data))[key] if isinstance(key, str) else data[key]
            )
            return row
        ...
        mock_user_rows = [
            create_mock_row(
                (1, 1, "xhs_cookie.json", "小红书账号1", 1, None, None, None), user_keys
            ),
            create_mock_row(
                (2, 2, "tencent_cookie.json", "视频号账号1", 1, None, None, None), user_keys
            ),
            create_mock_row(
                (3, 3, "douyin_cookie.json", "抖音账号1", 0, None, None, None), user_keys
            ),  # 数据库中已标记为异常
        ...
        with self.app.test_client() as client:
            ...
            mock_cursor.execute.assert_any_call(
                "SELECT type, COUNT(*) as count FROM user_info GROUP BY type"
            )
    ...
    def test_get_dashboard_stats_all_abnormal(self, mock_db_manager, mock_sqlite_connect):
        ...
        mock_user_rows = [
            create_mock_row(
                (1, 1, "xhs_cookie.json", "小红书账号1", 0, None, None, None), user_keys
            ),
            create_mock_row(
                (2, 2, "tencent_cookie.json", "视频号账号1", 0, None, None, None), user_keys
            ),
        ]
    ...
    def test_get_dashboard_stats_with_tasks(self, mock_db_manager, mock_sqlite_connect):
        ...
        mock_trend_rows = [
            ...
            create_mock_row(
                ((today - timedelta(days=1)).strftime("%Y-%m-%d"), "completed", 2), trend_keys
            ),
        ]
