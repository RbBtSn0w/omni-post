...
**Organization**: 任务按用户故事分组，支持独立实施和测试。针对新引入的崩溃恢复（FR-019）和账号互斥（FR-020）已补全任务。
...
- [x] T016a [FR-020] 实现 `apps/backend-node/src/services/lock-manager.ts` — 基于内存的账号级互斥锁（Mutex），防止并发操作同一账号 Context。
- [x] T017 实现 `apps/backend-node/src/app.ts` — Express 应用工厂。对标 `app.py`
- [x] T017a [FR-019] 在 `apps/backend-node/src/services/startup-service.ts` 中实现崩溃自愈扫描，启动时将 `uploading` 任务转为 `failed`。
- [x] T018 实现 `apps/backend-node/src/index.ts` — 服务器入口监听，接入 `startup-service` 并在启动时调用。对标 `run_app.py`
...
- [x] T031a [P] [US3] 定义 `apps/backend-node/src/uploader/base-uploader.ts` 抽象类/接口，规范所有平台上传行为。
...
- [x] T038 [US3] 实现 `apps/backend-node/src/services/publish-executor.ts` — 基于 Node.js 事件循环（setImmediate/Promises）的异步执行引擎。接入 `lock-manager` 确保同一账号物理发布互斥。
...
- [x] T046 [US5] **新增**：在 `apps/backend-node/tests/test_browser.test.ts` 中编写端到端 `stealth.min.js` 注入有效性隐式断言
- [x] T047 [P] [US5] 编写 Node 专属服务的测试方案对应 `test_login_service.test.ts`
- [x] T048 [P] [US5] 编写针对 `lock-manager` 和 `startup-service` 的单元测试 `test_executor_safety.test.ts`
...
- [x] T053 前端完整联调测试 — 记录实时环境下的并发稳定性。
- [x] T054 最终 Code Review，修正所有遗留的 TS `any` 定义。
...
- **Foundational (Phase 2)**: 依赖 Phase 1 完成 — **阻塞所有后续任务**
- **US1-US3**: 依赖 Phase 2 的安全底座（Lock Manager）和数据库能力
- **US5 (Phase 7)**: 依赖各模块初步实现后进行回归
- **Polish (Phase 8)**: 依赖所有用户故事完成

### Implementation Strategy
1. Foundation -> MVP (US 1)
2. **Critical**: 接入 Lock Manager 和 Startup Service 确保 Node 版的生产健壮性高于原版 Python。
